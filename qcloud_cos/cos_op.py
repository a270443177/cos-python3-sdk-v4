...
import qcloud_cos.cos_auth
from qcloud_cos.cos_err import CosErr
...
from qcloud_cos.cos_request import ListFolderRequest, DownloadFileRequest, MoveFileRequest
from qcloud_cos.cos_common import Sha1Util
...
class BaseOp(object):
    ...
    def _build_url(self, bucket, cos_path):
        ...
        cos_path = urllib.request.quote(cos_path, '~/')
    ...
    def del_base(self, request):
        ...
        auth = qcloud_cos.cos_auth.Auth(self._cred)
...
class FileOp(BaseOp):
    ...
    def _upload_slice_file(self, request):
        ...
        # 命中秒传
        if u'access_url' in control_ret['data']:
            return control_ret
        ...
        if request._max_con <= 1 or (
                'serial_upload' in control_ret['data'] and control_ret['data']['serial_upload'] == 1):
            ...
            from qcloud_cos.threadpool import SimpleThreadPool
            ...
            if not result['success_all']:
                return {'code': 1, u'message': str(result)}
